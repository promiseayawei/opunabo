...
import { attorneys, Particles } from "../page";
    ...
    <main className="bg-[#080C14] text-white overflow-x-hidden min-h-screen"
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <Image src={attorney.image} alt={attorney.name} fill sizes="100vw"
            ...
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="relative overflow-hidden max-w-md">
                ...
                {[["top-0 left-0","border-t-2 border-l-2"],["top-0 right-0","border-t-2 border-r-2"],["bottom-0 left-0","border-b-2 border-l-2"],["bottom-0 right-0","border-b-2 border-r-2"]].map(([pos, border], j) => (
                  <motion.div key={j} className={`absolute ${pos} w-10 h-10 ${border} border-[#FFD700] z-20 pointer-events-none`}
                    initial={{ opacity: 0, scale: 0.3 }} animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: j * 0.1 + 0.4, duration: 0.5 }} />
                ))}
                <Image src={attorney.image} alt={attorney.name} width={520} height={640}
      ...
      <section className="py-20 px-6 bg-[#060A10] border-t border-white/5">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
          ...
          {/* practice areas */}
          <motion.div className="bg-[#0E1420] p-8 border border-white/5 hover:border-[#FFD700]/20 transition-colors duration-500"
            ...
            <ul className="space-y-3">
              {attorney.practiceAreas.map((area, i) => (
                <motion.li key={i} className="flex items-start gap-3 text-sm text-gray-400 font-sans"
          ...
          <motion.div className="bg-[#0E1420] p-8 border border-white/5 hover:border-[#FFD700]/20 transition-colors duration-500"
            ...
            <ul className="space-y-4">
              {attorney.education.map((edu, i) => (
                <motion.li key={i} className="text-sm text-gray-400 font-sans leading-relaxed border-l border-[#FFD700]/20 pl-4"
                  initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.07 }}>
                  {edu}
                </motion.li>
          ...
          <motion.div className="bg-[#0E1420] p-8 border border-white/5 hover:border-[#FFD700]/20 transition-colors duration-500"
            ...
            <ul className="space-y-4">
              {attorney.achievements.map((item, i) => (
                <motion.li key={i} className="flex items-start gap-3 text-sm text-gray-400 font-sans leading-relaxed"
                  initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.07 }}>
                  <div className="flex-shrink-0 w-5 h-5 mt-0.5 border border-[#FFD700]/30 flex items-center justify-center">
                    <div className="w-1 h-1 bg-[#FFD700] rotate-45" />
                ...
                <motion.div key={other.slug}
                  ...
                  <Link href={`/team/${other.slug}`}
                    ...
                    <div className="relative flex-shrink-0 w-20 h-20 overflow-hidden">
                      <Image src={other.image} alt={other.name} fill sizes="80px"
                    ...
                    <ChevronRight size={16} className="text-[#FFD700]/30 group-hover:text-[#FFD700] group-hover:translate-x-1 transition-all duration-300 flex-shrink-0" />
                  </Link>
                </motion.div>
              ))}
            </div>
      ...
      <section className="py-24 px-6 relative overflow-hidden border-t border-white/5">
        ...
        <motion.div className="max-w-3xl mx-auto text-center"
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 leading-tight">
            Ready to Work With <span className="text-[#FFD700] drop-shadow-[0_0_20px_rgba(255,215,0,0.4)]">{attorney.name.split(" ")[1]}?</span>
          </h2>
